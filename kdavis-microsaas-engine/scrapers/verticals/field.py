"""
Auto repair / motor vehicle repair license database scraper (Source 2
for the Field vertical, agents/field_intel/agent.py). Targets
California's Bureau of Automotive Repair (BAR) public Auto Repair
Dealer license lookup (bar.ca.gov) -- picked as the first state to
implement following scrapers/verticals/real_estate.py's own precedent of
shipping one real, confirmed-public state source rather than guessing at
all seven states named in the Field agent's system prompt (TX/FL/CA/GA/
NC/AZ/OH). BAR's license search is public record, no login, standard for
auto repair shop verification.
...
BASE_SEARCH_URL = os.environ.get(
    "CA_BAR_LICENSE_SEARCH_URL", "https://www.bar.ca.gov/consumer/find_shop.php"
)
...
def _parse_results_table(html: str, location: str) -> list[RawLead]:
    ...
    for candidate in soup.find_all("table"):
        ...
        if "shop" in header_text or ("registration" in header_text and "business" in header_text):
            table = candidate
            break
    if table is None:
        return []
    ...
    for row in rows[1:]:
        ...
        leads.append(RawLead(
            ...
            title=record.get("title") or "Licensed Auto Repair Dealer",
            company=record.get("company"),
            location=record.get("location") or location,
            source="field_license_db",
...
class FieldScraper(BaseScraper):
    source_name = "field_license_db"
    ...
    def scrape(self, location: str, filters: dict) -> list[RawLead]:
        # e.g. ["Auto Repair Dealer", "Smog Check Station"] -- filters
        # carries whatever this scraper understands and ignores the rest
        # (BaseScraper's own contract).
        registration_types = filters.get("license_types") or ["Auto Repair Dealer"]
        leads: list[RawLead] = []
        ...
        for registration_type in registration_types:
            ...
            try:
                response = self._get(
                    self.base_url,
                    params={"City": location, "RegistrationType": registration_type},
                    timeout=15,
...
def scrape(location: str, filters: dict) -> list[RawLead]:
    ...
    return FieldScraper().scrape(location, filters)
