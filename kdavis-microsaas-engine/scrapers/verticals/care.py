"""
Childcare license database scraper (Source 2 for the Care vertical,
agents/care_intel/agent.py). Targets Texas Health and Human Services'
public Child Care Search (childcare.hhs.texas.gov) -- picked as the
first state to implement following scrapers/verticals/real_estate.py's
own precedent of shipping one real, confirmed-public state source rather
than guessing at all seven states named in the Care agent's system
prompt (TX/FL/CA/NY/GA/NC/OH). Texas's provider search is public record,
no login, standard for licensed childcare operator verification.
...
Remaining six states (FL/CA/NY/GA/NC/OH) are follow-on work, one file (or
...
BASE_SEARCH_URL = os.environ.get(
    "TX_HHS_CHILDCARE_SEARCH_URL", "https://childcare.hhs.texas.gov/Public/PublicSearch.aspx"
)
...
def _parse_results_table(html: str, location: str) -> list[RawLead]:
    ...
    for candidate in soup.find_all("table"):
        ...
        if "operation" in header_text or ("permit" in header_text and "capacity" in header_text):
            table = candidate
            break
    if table is None:
        return []
    ...
    for row in rows[1:]:
        ...
        leads.append(RawLead(
            ...
            title=record.get("title") or "Licensed Childcare Operator",
            company=record.get("company"),
            location=record.get("location") or location,
            source="care_license_db",
...
class CareScraper(BaseScraper):
    source_name = "care_license_db"
    ...
    def scrape(self, location: str, filters: dict) -> list[RawLead]:
        # e.g. ["Licensed Center", "Licensed Child-Care Home"] -- filters
        # carries whatever this scraper understands and ignores the rest
        # (BaseScraper's own contract).
        operation_types = filters.get("license_types") or ["Licensed Center"]
        leads: list[RawLead] = []
        ...
        for operation_type in operation_types:
            ...
            try:
                response = self._get(
                    self.base_url,
                    params={"City": location, "OperationType": operation_type},
                    timeout=15,
...
def scrape(location: str, filters: dict) -> list[RawLead]:
    ...
    return CareScraper().scrape(location, filters)
