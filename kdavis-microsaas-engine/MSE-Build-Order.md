...
- **SATURATED → DO_NOT_BUILD:** ✅ confirmed repeatedly (10/10 real opportunities now — 6 under v2.0, 4 under v3.0 across two batches, including the niche-targeted Legal/Finance batch below)
- **CLEAR → BUILD:** ❌ not yet confirmed — never seen in a real run
- **PARTIAL → BUILD (approved differentiation thesis):** ❌ not yet confirmed — the whole point of adding this state was to loosen v2.0's false SATURATED calls, but no real run has landed here yet
- **RESUBMIT → needs_correction:** ❌ not yet confirmed *under v3.0 specifically* (a v2.0-era RESUBMIT was seen once, on different rules/schema — doesn't count as a v3.0 confirmation)
