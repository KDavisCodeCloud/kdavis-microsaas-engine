...
**Real fix:** reordered `main.py` so `CORSMiddleware` is registered last (outermost), wrapping every response — success or error. Verified locally and against live production (`mse-api` on Railway) that both a real 401 and a real 403 now carry the correct `Access-Control-Allow-Origin` header, not just the OPTIONS preflight. **Still not yet confirmed by an actual click in the live dashboard — please try the buttons again.** If it fails again, it should now show a real, specific error message instead of "Failed to fetch" — that message is the next diagnostic clue if there's still an issue (e.g. it would mean the frontend's session JWT itself isn't being accepted, a separate problem from the CORS layer this fixed).
